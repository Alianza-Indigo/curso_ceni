...
export default function AdminLoginForm() {
  const [state, formAction, pending] = useActionState(loginAdminAction, undefined);

  return (
    <form action={formAction} className="mt-8 w-full space-y-3">
      <div>
      ...
      {state?.error && (
        <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-[#7a2b2b]">{state.error}</p>
      )}
