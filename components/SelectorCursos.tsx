        ...
        {cursos.map(({ curso, completados, total }) => {
          ...
          return (
            <div key={curso.id} className="flex flex-col">
              <Link
                href={`/${curso.slug}`}
                className="group flex flex-1 flex-col rounded-2xl border border-[#e5def4] bg-white p-7 shadow-sm transition-colors hover:border-[#4b18a8]"
              >
              ...
              {curso.landing && (
                <a
                  href={curso.landing}
                  className="mt-2 inline-flex items-center gap-1 self-start px-1 text-sm font-bold text-[#6c6690] hover:text-[#4b18a8] hover:underline"
                >
                  Ver presentación del curso <ArrowRight className="h-3.5 w-3.5" />
                </a>
              )}
            </div>
          );
